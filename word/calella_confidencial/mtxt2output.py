...
def process_replace_tex(text):
    ...
    subdict = OrderedDict(
        [
            ...
            (
                r"^==([^=]+)==\s*(label{\w+})?\s*$",
                {"r": r"\\subsection{\1}", "flags": re.MULTILINE},
            ),
            (
                r"^--([^-]+)--\s*(label{\w+})?\s*$",
                {"r": r"\\subsection*{\1}", "flags": re.MULTILINE},
            ...
            (
                r"^---([^-]+)---\s*(label{\w+})?\s*$",
                {"r": r"\\subsubsection*{\1}", "flags": re.MULTILINE},
            ),
            ...
            (r"/~", {"r": r"\\begin{em}", "flags": re.MULTILINE}),
            (r"~/", {"r": r"\\end{em}", "flags": re.MULTILINE}),
            (r"/!", {"r": r"\\begin{normalfont}", "flags": re.MULTILINE}),
            (r"!/", {"r": r"\\end{normalfont}", "flags": re.MULTILINE}),
    ...
    def replfn(adict, r, matchobj):
        if r.find("footnote") != -1:
            ...
            if matchobj.group(0).count("|") in [2, 3, 4]:
                ...
                return (
                    ""
                    + matchobj.group(1)
                    + "\\footnote{{{}}}".format(re.sub(r"<br\s*/>", ", ", tr[1]))
                )
        ...
        if r.find("endnote") != -1 and matchobj.group(0).count("|") == 1:
            adict["counter"] += 1
            adict["alist"].append(
                (adict["counter"], matchobj.group(1), matchobj.group(2))
            )
...
def process_replace_html(text):
    ...
    subdict = OrderedDict(
        [
            ...
            (
                r"[\s\[\"]\[([^|]+)\|([^\]]+)\]",
                {"r": r'<abbr class="translation", title="\1">\2</abbr>', "flags": 0},
            ),
            ...
            (
                r"^=([^=]+)=\s*(label{\w+})?\s*$",
                {"r": r"<h1<\1</h1>", "flags": re.MULTILINE},
            ),
            (
                r"^-([^-]+)-\s*(label{\w+})?\s*$",
                {"r": r"<h1>\1</h1>", "flags": re.MULTILINE},
            ...
            (
                r"^==([^=]+)==\s*(label{\w+})?\s*$",
                {"r": r"<h2>\1</h2>", "flags": re.MULTILINE},
            ),
            (
                r"^--([^-]+)--\s*(label{\w+})?\s*$",
                {"r": r"<h2>\1</h2>", "flags": re.MULTILINE},
            ...
            (
                r"^---([^-]+)---\s*(label{\w+})?\s*$",
                {"r": r"<h3>\1</h3>", "flags": re.MULTILINE},
            ),
            # Spiegelstrich
            ...
            if matchobj.group(0).count("|") in [2, 3]:
                ...
                if matchobj.group(2).count("|") == 2:
                    try:
                        ...
                        print(tr)
                        raise
                return ' <abbr title="{1}">{0}</abbr>'.format(
                    matchobj.group(1), "{}".format(re.sub(r"<br\s*/>", ", ", tr[1]))
                )
                return (
                    " "
                    + matchobj.group(1)
...
def make_cleanup(args):
    ...
    def prepare(text):
        ...
        text = re.sub(
            r"^-\s*([a-z])(.*)",
            lambda m: "- {}{}".format(m.group(1).upper(), m.group(2)),
            text,
            flags=re.M,
        )
...
def make_dictionary(args):
    with open("_words.txt") as fh:
        ...
        for index, line in enumerate(wordlist):
            ...
            esword = wlist[0]
            if (
                esword.startswith("el ")
                or esword.startswith("la ")
                or esword.startswith("los ")
            ...
            if firstletter not in firstletterlist:
                ...
                outlist.append(
                    "\\hypertarget{{a{}}}{{\\section*{{{}}}}}".format(
                        len(firstletterlist), firstletter
                    )
                )
            outlist.append(
                "\\textbf{{{0}}}\\enspace--\\enspace{{{1}}}\\hfill~\\\\".format(*wlist)
            )
    link = " - ".join(
        "\\hyperlink{{a{}}}{{{}}}".format(i, a) for i, a in enumerate(firstletterlist)
    )
...
def process_tex(args):
    ...
    for search in rdict:
        ...
        template = re.sub(
            f"\\\\{search}{{}}", f"\\\\{search}{{{rdict[search]}}}", template, re.M
        )
...
def process_ebook(args):
    ...
    book.add_metadata(
        "DC",
        "description",
        "This is my private version of '{}' from {}".format(title, author),
    )
    ...
    for index, chapter in enumerate(textlist):
        chaptertitle = (
            "Capítulo {}".format(index + 1)
            if args.config.get("chapterlist", None) is None
            else args.config["chapterlist"][index]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--with-link", action="store_true", help="create links in dictionary"
    )
    parser.add_argument(
        "--configfile",
        "-c",
