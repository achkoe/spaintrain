...
def process(args):
    subdict = {
        ...
        u"\[\[([^\]]+)\]\]": {"r": ur"\\footnote{\1}", "flags": 0},
        # u"{{([^}]+)}}": {"r": ur"\\mnote{\1}", "flags": 0, "out": args.printvocabulary},
        #u"{{([^|]+)\|([^}]+)}}": {"r": ur"\1\\mnote{\2}", "flags": 0, "out": args.printvocabulary},
        u"{{([^|]+)\|([^}|]+)(.*)}}": {"r": ur"\1\\mnote{\2}", "flags": 0, "out": args.printvocabulary},
    ...
    def replfn(r, matchobj):
        if "out" in r and r["out"]:
            print(u"{}|{}{}".format(matchobj.group(1), matchobj.group(2), matchobj.group(3)))
        if r['r'].find("section") != -1 and len(matchobj.groups()) > 1 and matchobj.group(2):
            #print( matchobj.expand(r) + "\\" + matchobj.group(2))
            return matchobj.expand(r['r']) + "\\" + matchobj.group(2)
        return matchobj.expand(r['r'])

    with codecs.open(args.infile, 'r', "utf-8") as fh:
        text = fh.read()
    for key, replacement in subdict.iteritems():
        rf = partial(replfn, replacement)
        text = re.sub(key, rf, text, flags=replacement['flags'])
    linelist = text.splitlines()[3:]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--printvocabulary", "-p", help="print vocabulary", action="store_true")
